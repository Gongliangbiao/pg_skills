...
每个 pack 描述一个 PostgreSQL 主题如何展开为可复用的测试覆盖。pack 只用于宽泛主题规划、检查遗漏或补充覆盖维度；它不是 ready case design 的替代品。pack 只放主题相关的覆盖规则；共享的文件格式和输出规则放到 `output-profiles/`。
...
- `sql-only`：该主题最终可生成 SQL 文件，expected 由外部框架采集和比对。实际 SQL 生成仍需要 ready case design。
- `error-output`：包含预期错误输出。
- `isolation-schedule`：更适合 isolation schedule，而不是普通 SQL regression。

## Pack 选择规则

选择触发短语和主题描述最匹配用户请求的 pack。只有当输入是宽泛主题、覆盖规划、遗漏检查或 case design 明确要求补充覆盖时，才选择 pack。
...
- 不要在 pack 中定义最终文件名或数据库对象名。
- 不要用 pack 覆盖 ready case design 已经明确的执行设计和预期结果。
