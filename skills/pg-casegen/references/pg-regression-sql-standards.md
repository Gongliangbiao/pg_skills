...
数据库对象名必须遵守 `pg-sql-case-naming` 的短对象名规则，避免超过 PostgreSQL 标识符长度限制。

使用 3 到 4 个字符的短前缀区分对象类型，前缀要能直接看出对象含义；不要使用 `i_`、`v_`、`s_`、`fn_` 这类过短缩写。
...
主名称应从 case ID 和官方章节推导，并尽量贴近用例名。SQL 对象名使用小写 snake_case；只有接近或超过 PostgreSQL 63 字节标识符限制、或同一用例内重名时，才缩写到建议的 48 字符以内。

默认不要追加无语义哈希后缀。只有对象名缩写后可能重名或超过长度限制时，才追加稳定短哈希，并在 SQL header 或 case design 中说明该后缀仅用于防重。
...
AB 多会话用例默认只在 A 文件 setup 阶段执行 `DROP TABLE IF EXISTS` 清理上次残留，不在用例末尾重复 `DROP TABLE`。除非 case design 明确要求验证 cleanup 行为，否则不要生成“清理测试对象”收尾步骤。
...
- 查询列和行数应尽量少，只验证必要内容。
- 当测试表数据量不多、单行数据不长，且输出不会引入不稳定字段时，优先使用 `SELECT * FROM <table> ORDER BY <key>`，让 expected 更直观看到完整行状态。
- 当表包含大字段、环境敏感字段、时间戳、OID、随机值或非测试目标列时，显式选择必要列。
- 涉及事务提交、回滚或可见性变化的用例，应在关键事务结束前后各安排一次稳定 `SELECT`，用于确认提交/回滚前后表中数据状态。并发场景中如果提交后的状态会被对端继续修改，应把最终状态验证放在对端完成同步之后。

## 21. 表结构多样性

不要所有用例都套用完全相同的表结构。表结构应由测试点语义确定，并保持确定性，而不是真正随机。

建议按场景选择不同结构：

- MVCC 可见性：可使用文本、时间戳、状态列等易观察字段。
- 行锁和 `UPDATE` 重检：可使用布尔条件列、计数字段、唯一键或检查约束。
- 外键/唯一性冲突：应包含主键、唯一索引、外键或约束名。
- 序列或事务 ID：应包含 sequence、identity、bigint 或 xid 观测列。
- 锁模式或索引相关场景：可增加不同索引类型或多列索引，但不要引入与测试点无关的复杂性。

同一章节连续生成多个用例时，应尽量避免完全相同的列名、列类型和索引组合；但变化必须服务测试目标，不能为了“随机”制造难以理解的数据结构。
